namespace :db do
  namespace :populate do
    ...
    task :user_station_artists => :environment do      
      ENV['conditions'] = "abstract_station_id IN (#{ENV['artists'].split(/,/).map { |a| Artist.find_by_slug(a).station.id }.join(',') })" if ENV.has_key?('artists')
    ...
    task :station_artists => :environment do
      ENV['conditions'] = "id IN (#{ENV['artists'].split(/,/).map { |a| Artist.find_by_slug(a).station.id }.join(',') })" if ENV.has_key?('artists')
