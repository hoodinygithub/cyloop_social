class TranslateController < ActionController::Base
  ...
  def translate
    ...
    redirect_to params.slice(:filter, :sort_by, :key_type, :key_pattern, :text_type, :text_pattern, :from_locale, :to_locale).merge({:action => :index})
  end
  ...
  def filter_by_text_pattern
    ...
    @keys.reject! do |key|
      from_l = lookup(@from_locale, key)
      to_l   = lookup(@to_locale, key)
            
      case params[:text_type]
      when 'contains':
        (!from_l.present? || !from_l.to_s.downcase.index(params[:text_pattern].downcase)) &&
        (!to_l.present? || !to_l.to_s.downcase.index(params[:text_pattern].downcase))
      when 'equals':
        (!from_l.present? || from_l.to_s.downcase != params[:text_pattern].downcase) &&
        (!to_l.present? || to_l.to_s.downcase != params[:text_pattern].downcase)
      else
