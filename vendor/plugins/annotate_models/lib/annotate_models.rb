...
module AnnotateModels
  ...
  def self.do_annotations
    header = PREFIX.dup
    # Waffle patch to prevent schema version from appearing, adds too much noise on commits when rerun.
    ...
    self.get_model_names.each do |m|
      ...
      begin
        ...
        if klass < ActiveRecord::Base && !klass.abstract_class?
          puts "Annotating #{class_name}" if $VERBOSE
          self.annotate(klass, header)
        else
          puts "Skipping #{class_name}" if $VERBOSE
        end
      rescue Exception => e
