        ...
        unless attachment.original_filename.blank?
          unless valid_types.blank?
            ...
            unless valid_types.any?{|t| content_type.nil? || t === content_type }
              options[:message] || I18n.t('activerecord.errors.messages.invalid_content_type')
            end
          end
        end
      end
  ...
  module InstanceMethods #:nodoc:
    ...
    def save_attached_files
      each_attachment do |name, attachment|
        if (send("#{name}_file_name_changed?"))
          logger.info("[paperclip] Saving attachment: #{name}: #{attachment}, self: " + send("#{name}_file_name_changed?").to_s)
