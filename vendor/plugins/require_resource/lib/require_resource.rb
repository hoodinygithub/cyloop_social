...
module RequireResource
  ...
  def javascript_auto_include_tags(options = {})
    ...
    tags = @javascripts.collect { |s| javascript_include_tag(s) }
    tags.uniq.inject("") { |buffer, js| buffer << js + "\n    " }
  end
  ...
  protected
    ...
    def autorequire(resource_type, extension = nil)
      ...
      while ![ActionController::Base].include? class_iterator
        ...
        candidates |= candidates_for(controller_path, controller.action_name)
        class_iterator = class_iterator.superclass
      end
      
      for candidate in candidates
        if FileTest.exist?("#{resource_path}/#{candidate}.#{extension}")
          next if candidate =~ /application/
          require_resource(resource_type, candidate)
        end
      end
    end
    
    def candidates_for(controller_path, action_name)
      candidates = []
      candidates << "#{controller_path}"
      candidates << "#{controller_path}/#{controller.action_name}"
      candidates << "#{controller_path}/new"  if action_name == "create"
      candidates << "#{controller_path}/edit" if action_name == "update"
      candidates
    end
