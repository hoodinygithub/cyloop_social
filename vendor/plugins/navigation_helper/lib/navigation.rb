...
module RPH
  module Navigation
    ...
    module InstanceMethods
      ...
      def filter(sections, options={})
        navigation, items = Navigator.new(sections, options), []

        navigation.sections.each do |link|
          if link == controller.class.current_filter
            css = 'filter active'
          else
            css = 'filter'
          end
          
          items << construct_styles(navigation, link, css)
        end
        items
      end
      ...
      def construct_styles(nav, section, css)
        text = nav.text_for(section)
        path = ROUTES[section.to_sym] || send("#{section.to_s.downcase}_path")
        link = link_to(content_tag(:span, text), path, :class => css.to_s)
    ...
    module ClassMethods
      ...
      def current_filter(tab=nil)        
        self._current_filter = tab if tab
