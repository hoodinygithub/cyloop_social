...
class Artist < Account
  ...
  def top_stations(limit=10)
    result = []
    if station
      result = station.user_stations.all(:limit => limit, :order => 'user_stations.total_plays DESC') 
    end
    result
  end
